...
要求 Node.js 20+。使用 AI 简历改写（由本机代理转交 Codex CLI 配置的模型）或手动触发官网岗位搜索时，需要已安装并登录的 Codex CLI。
...
- 不会自动填表或提交。字段包明确排除工作授权、签证/赞助、薪资、EEOC/身份、保密声明及提交动作；每次成功打开后只在浏览器本地记录不含明文的字段来源指纹和授权审计。
